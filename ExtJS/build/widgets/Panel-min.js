...
Ext.Panel=Ext.extend(Ext.Container,{baseCls:'x-panel',collapsedCls:'x-panel-collapsed',maskDisabled:true,animCollapse:Ext.enableFx,headerAsText:true,buttonAlign:'right',collapsed:false,collapseFirst:true,minButtonWidth:75,elements:'body',resetBodyCss:false,toolTarget:'header',collapseEl:'bwrap',slideAnchor:'t',disabledClass:'',deferHeight:true,expandDefaults:{duration:.25},collapseDefaults:{duration:.25},initComponent:function(){Ext.Panel.superclass.initComponent.call(this);this.addEvents('bodyresize','titlechange','iconchange','collapse','expand','beforecollapse','beforeexpand','beforeclose','close','activate','deactivate');if(this.unstyled){this.baseCls='x-plain';}
...
if(this.resetBodyCss===true){el.addClass('x-panel-reset');}
...
if(this.fbar){this.fbar=Ext.create(this.fbar,'toolbar');this.fbar.enableOverflow=false;if(this.fbar.items){this.fbar.items.each(function(c){c.minWidth=c.minWidth||this.minButtonWidth;},this);}
...
t.addClassOnOver(overCls);}}},onLayout:function(){if(this.topToolbar){this.topToolbar.doLayout();}
...
if(this.fbar){var fWidth=1,f=this.fbar;if(this.buttonAlign=='left'){fWidth=w-f.container.getFrameWidth('lr');}else if(Ext.isIE&&!Ext.isIE8){var el=f.getEl();if(Ext.isIE7&&Ext.isStrict){(function(){f.setWidth(el.child('.x-toolbar-ct').getWidth());}).defer(1)}else{fWidth=el.getWidth();}}else{fWidth='auto';}
f.setWidth(fWidth);}
